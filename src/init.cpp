...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/eszett.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specified in eszettd / RPC client
            std::string strUsage = _("EsZett version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  eszettd [options]                     " + "\n" +
                  "  eszettd [options] <command> [params]  " + _("Send command to server or eszettd") + "\n" +
                  "  eszettd [options] help                " + _("List commands") + "\n" +
                  "  eszettd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "eszett:"))
                fCommandLine = true;
...
{
    ThreadSafeMessageBox(str, _("EsZett"), wxOK | wxMODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    ThreadSafeMessageBox(str, _("EsZett"), wxOK | wxICON_EXCLAMATION | wxMODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>          "   + _("Specify configuration file (default: eszett.conf)") + "\n" +
        "  -pid=<file>           "   + _("Specify pid file (default: eszettd.pid)") + "\n" +
    ...
    strUsage += string() +
        _("\nSSL options: (see the EsZett Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-reservebalance")) // eszett: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // eszett: checkpoint master priv key
    {
    ...
    // Make sure only a single EsZett process is using the data directory.
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  EsZett is probably already running."), pszDataDir));
    ...
    //
    if (fDaemon)
        fprintf(stdout, "EsZett server starting\n");
    ...
    if (mapArgs.count("-onlynet")) {
        ...
        BOOST_FOREACH(std::string snet, mapMultiArgs["-onlynet"]) {
            ...
            if (net == NET_UNROUTABLE) {
                ThreadSafeMessageBox(_("Unknown network specified in -onlynet"), _("EsZett"), wxOK | wxMODAL);
                return false;
            }
            nets.insert(net);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of EsZett") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart EsZett to complete") << "\n";
